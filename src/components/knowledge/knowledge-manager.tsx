...
type Document = { id: string; title: string; source_type: string; file_size: number; status: string; created_at: string };
...
export function KnowledgeManager() {
  ...
  const [error, setError] = useState(""); const [limitMessage, setLimitMessage] = useState("");
  ...
  async function remove(id: string) { if (!window.confirm("Delete this knowledge source?")) return; await fetch(`/api/documents/${id}`, { method: "DELETE" }); await loadDocuments(); }
  ...
  return <div className="space-y-6">
    ...
    {limitMessage ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/35 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="upgrade-title"><div className="relative w-full max-w-md rounded-3xl bg-surface p-7 text-center shadow-2xl ring-1 ring-border"><button className="absolute right-4 top-4 rounded-lg p-2 text-muted hover:bg-surface-muted" onClick={() => setLimitMessage("")} aria-label="Close upgrade dialog">×</button><div className="mx-auto grid size-14 place-items-center rounded-2xl bg-accent-soft text-2xl text-accent">✦</div><h2 id="upgrade-title" className="mt-5 text-xl font-bold">You&apos;ve reached your plan limit</h2><p className="mt-2 text-sm leading-6 text-muted">{limitMessage}</p><p className="mt-2 text-sm text-muted">Upgrade your plan to keep adding company knowledge.</p><div className="mt-6 flex flex-col gap-2"><Link href="/app/billing" className="inline-flex h-11 items-center justify-center rounded-xl bg-accent px-4 text-sm font-bold text-white hover:bg-accent-hover">View plans <span className="ml-2">→</span></Link><button className="h-10 rounded-xl text-sm font-semibold text-muted hover:bg-surface-muted" onClick={() => setLimitMessage("")}>Maybe later</button></div></div></div> : null}
    <Card><CardContent className="p-0">{documents.length ? <div className="divide-y divide-border">{documents.map((document) => <div key={document.id} className="flex items-center gap-4 px-5 py-4 sm:px-6"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-accent-soft text-xs font-bold uppercase text-accent">{document.source_type === "text" ? "Aa" : document.source_type}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{document.title}</p><p className="mt-1 text-xs text-muted">{formatSize(document.file_size)} <span className="mx-1">•</span> Added {new Date(document.created_at).toLocaleDateString()}</p></div><Badge tone="success">{document.status}</Badge><button onClick={() => void remove(document.id)} className="rounded-lg p-2 text-muted hover:bg-red-50 hover:text-danger" aria-label={`Delete ${document.title}`}>×</button></div>)}</div> : <div className="px-6 py-16 text-center"><div className="mx-auto grid size-14 place-items-center rounded-2xl bg-accent-soft text-2xl text-accent">✦</div><h2 className="mt-5 font-bold">Your knowledge base is empty</h2><p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-muted">Upload your first document or paste your FAQs. Your assistant will use them to answer questions.</p></div>}</CardContent></Card>
  </div>;
}
